...
k17 = randi([5 10],1,7); %choose random, dense dataset
k2 = randi([8 15],1,7);
k34 = randi([10 20],1,7);
k56 = randi([-5,20],1,7);
...
csvwrite('dataset_fk3.csv',pose_end_effector);

disp('Forward kinematics done');

%% Storing theta to csv

theta = [theta_1' theta_2' theta_3' theta_4' theta_5' theta_6' theta_7'];
csvwrite('dataset_thetas3.csv',theta);

%% extracting csv data 
data_from_csv = readmatrix('dataset_fk3.csv');
theta_from_csv = readmatrix('dataset_thetas3.csv');
...
net1_model = train(net1,train_partition_1(:,1:6)',train_partition_1(:,7)', 'useGPU','yes');
predicted_Theta_1=sim(net1_model, test_partition_1(:,1:6)', 'useGPU','yes');
...
net2_model = train(net1,train_partition_2(:,1:6)',train_partition_2(:,7)', 'useGPU','yes');
predicted_Theta_2=sim(net2_model, test_partition_2(:,1:6)', 'useGPU','yes');
...
net3_model = train(net3,train_partition_3(:,1:6)',train_partition_3(:,7)', 'useGPU','yes');
predicted_Theta_3=sim(net3_model, test_partition_3(:,1:6)', 'useGPU','yes');
...
net4_model = train(net4,train_partition_4(:,1:6)',train_partition_4(:,7)', 'useGPU','yes');
predicted_Theta_4=sim(net4_model, test_partition_4(:,1:6)', 'useGPU','yes');
...
net5_model = train(net5,train_partition_5(:,1:6)',train_partition_5(:,7)', 'useGPU','yes');
predicted_Theta_5=sim(net5_model, test_partition_5(:,1:6)', 'useGPU','yes');
...
net6_model = train(net6,train_partition_6(:,1:6)',train_partition_6(:,7)', 'useGPU','yes');
predicted_Theta_6=sim(net6_model, test_partition_6(:,1:6)', 'useGPU','yes');
...
net7_model = train(net7,train_partition_7(:,1:6)',train_partition_7(:,7)', 'useGPU','yes');
predicted_Theta_7=sim(net7_model, test_partition_7(:,1:6)', 'useGPU','yes');
...
%% FK using predicted thetas

verification_config = q;

for i = 1:117649
    for j = 1:7
        verification_config(i,j).JointPosition = visual_data_calculated(i,j); 
    end
end

for i = 1:(7^6)
    T_verif = getTransform(cassie,verification_config(i,:),'left_toe');  
    P87 = [.0047;.0275;-0.0001;1];
    P81_verif(i,:) = [T_verif*P87]';
    position_verif = P81_verif(i,1:3);
    eul81_verif(i,:) = [tform2eul(T_verif)];
    orientation_verif = eul81_verif(i,1:3);
    pose_end_effector_verif(i,:) = [position_verif orientation_verif];
end

csvwrite('dataset_fk3_verif.csv',pose_end_effector_verif);

disp('Forward kinematics verification done');
%% Min Max of FK by NN

minx_verif = min(position_verif(:,1))
miny_verif = min(position_verif(:,2))
minz_verif = min(position_verif(:,3))

maxx_verif = max(position_verif(:,1))
maxy_verif = max(position_verif(:,2))
maxz_verif = max(position_verif(:,3))